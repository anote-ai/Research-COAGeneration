...
\subsection{BattleCOA Terminology}

The BattleCOA Boot Camp material describes two relevant DecisionFunctions: Match Effectors (MEF), which searches for BattleAssets that can achieve a requested BattleEffect and returns ranked EffectEffectorMatches, and Generate BattleCOA (GBC), which uses those matches as seeds for constructing BattleCOA hypergraphs. Our current implementation is a simplified research harness and does not implement either DecisionFunction directly. Earlier drafts used MEF and GBC as names for internal scalar metrics; this draft avoids that collision and calls them the synthetic COA quality score and BLUE-vs-RED advantage score.
...
MEF and GBC (introduced below) are named after the Match Effectors and Generate BattleCOA DecisionFunctions in the Transformational Model for Decision Advantage (TM-DA)~\cite{tmda2025bootcamp}, an operational battle-management framework that independently uses these acronyms for a richer, graph-structured pair of processes. COA-Bench's MEF and GBC are coarse, single-scalar analogues -- reused without modification from the existing \texttt{coageneration} formulas -- not implementations of TM-DA's effector ranking or hypergraph plan assembly. See \path{papers/coa-bench-aaai2027/main.tex} in this repository for the full stated correspondence and its limits.
=======
A COA is a typed object: an ordered or chained set of \texttt{Action}s (each with a category, target, priority, and optional tool call), an optional conditional branch, an objective string, a force assignment (BLUE or RED), a domain tag, and a scalar synthetic quality score in $[-1, 1]$ combining effectiveness, cost, and risk. Conditional branches and tool calls let a COA represent doctrinally meaningful structures -- e.g., a reconnaissance action gating a strike-or-withdraw branch -- rather than a flat action list.
>>>>>>> 36606dc99a5d4a505afe515467c39e2bb4686ff7

\subsection{Self-Play and Sampled Best-Response}

Given an opposing force's COA, a policy generates a response COA for the same scenario's \texttt{GameState}. The baseline policy (\texttt{SelfPlayEngine.best\_response}) draws one random response. The \texttt{SampledBestResponsePolicy} draws $n$ candidate responses and either returns the single highest-quality candidate (\texttt{generate\_coa}) or exposes all $n$ candidates for downstream comparison (\texttt{generate\_candidates}). This paper compares these two response modes directly.

\subsection{Pairwise and Multi-COA Evaluation}

Two opposing COAs are scored with a BLUE-vs-RED advantage score, $\mathrm{Advantage} = (q_{\mathrm{blue}} - q_{\mathrm{red}} + 2)/4 \in [0,1]$, and a Nash-gap distance, $|\,\mathrm{payoff}_{\mathrm{blue}} + \mathrm{payoff}_{\mathrm{red}} - 1\,|$, measuring deviation from a zero-sum equilibrium after quality scores are rescaled to $[0,1]$ payoffs. A set of candidate COAs for one scenario (e.g., the $n$ samples from \texttt{SampledBestResponsePolicy}) can be compared with \texttt{compare\_coas}, which ranks candidates by quality score, computes each candidate's heuristic doctrinal alignment, measures action-type diversity (mean pairwise Jaccard distance), and identifies the Pareto-optimal subset on (quality, doctrinal alignment).

\subsection{Doctrinal Rubric and Wargame Check}

\texttt{fm30\_rubric\_scores} extracts six heuristic, doctrine-inspired terms from a COA -- objective clarity, intelligence preparation, combined-arms balance, sustainment, risk mitigation, and tempo/sequencing -- each in $[0,1]$, from the COA's objective text and action categories. \texttt{doctrinal\_alignment\_score} is their weighted aggregate. \texttt{lanchester\_wargame\_outcome} runs a stylized multi-step attrition simulation in which each side's effective combat power is its raw asset capability scaled by $1 + q + \mathrm{doctrinal\_alignment}$, where $q$ is the synthetic quality score, so a COA's quality directly affects the wargame outcome rather than only the COA-level metrics.
...
=======
BLUE advantage score & .521 [.512, .529] & .488 [.482, .494] \\
...
\end{figure}
=======
Table~\ref{tab:main} shows a consistent picture across four independent metrics: letting RED sample 8 candidate responses and keep its highest-quality one makes RED a measurably tougher opponent. The BLUE advantage score falls from .521 to .488 because RED's quality score rises under best-of-8 selection. The Nash-gap distance from equilibrium grows (.194 to .259) because RED's payoff increases without a corresponding adjustment to BLUE's fixed COA -- consistent with the gap measuring deviation from a balanced exchange rather than tracking a converged equilibrium. BLUE's win rate in the wargame check falls from .967 to .833, an independent confirmation using a different mechanism (attrition simulation rather than the quality scores directly). Notably, RED's doctrinal alignment is \emph{higher} under best-of-8 selection (.671 vs.\ .651) rather than lower: selecting purely for quality score in this generator did not come at the cost of doctrinal coherence, at least for this synthetic action space. We treat this as a narrow, generator-specific finding rather than a general claim that quality-greedy selection preserves doctrinal alignment.

\subsection{Multi-COA Comparison}

Applying \texttt{compare\_coas} to the 8 sampled RED candidates per scenario gives a mean candidate diversity (mean pairwise Jaccard distance of action types) of .734 [.714, .756], a mean quality-score spread of .242 [.226, .258], and a mean of 2.20 [1.87, 2.53] Pareto-optimal candidates out of 8 on the (quality, doctrinal alignment) frontier. These numbers indicate that the 8 sampled candidates are not redundant: roughly a quarter of them are simultaneously non-dominated on effectiveness and doctrinal alignment, which is the situation \texttt{compare\_coas} is designed to surface for a human planner choosing among several proposed COAs rather than a single auto-selected one.
>>>>>>> 36606dc99a5d4a505afe515467c39e2bb4686ff7
...
The experiment script writes three output files to \path{results/coa-bench/main/}: \texttt{rows.csv} (one row per scenario with all per-scenario metrics), \texttt{summary.csv} (the bootstrap-CI summary reported in Table~\ref{tab:main} and Sections~4.3--4.5), and \texttt{details.json} (run configuration including the model identifier and whether LLM policy was active). The random stream is keyed by base seed; all reported numbers are reproducible bit-for-bit given the same library version.
