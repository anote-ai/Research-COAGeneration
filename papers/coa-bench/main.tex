...
\begin{table}[t]
\caption{Single-sample vs.\ sampled best-response RED, over 90 scenarios. CI is the 95\% bootstrap interval.}
\label{tab:main}
\small
\begin{tabular}{lrr}
\toprule
...
\midrule
GBC (blue advantage) & .524 [.519, .530] & .490 [.487, .494] \\
Nash gap & .199 [.189, .209] & .267 [.261, .274] \\
Red doctrinal align. & .667 [.640, .695] & .678 [.658, .700] \\
Blue win rate (wargame) & .900 & .811 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:main} and Figure~\ref{fig:policy} show a consistent picture across four independent metrics over 90 scenarios: letting RED sample 8 candidate responses and keep its best-MEF one makes RED a measurably tougher opponent. GBC, which is computed from BLUE's MEF advantage over RED, falls from .524 to .490 because RED's MEF score rises under best-of-8 selection. The Nash-gap distance from equilibrium grows (.199 to .267) because RED's payoff increases without a corresponding adjustment to BLUE's fixed COA -- consistent with the gap measuring deviation from a balanced exchange rather than tracking a converged equilibrium. BLUE's win rate in the wargame check falls from .900 to .811, an independent confirmation using a different mechanism (attrition simulation rather than the MEF-based scores directly). Notably, RED's doctrinal alignment is \emph{higher} under best-of-8 selection (.678 vs.\ .667) rather than lower: selecting purely for MEF score in this generator did not come at the cost of doctrinal coherence, at least for this synthetic action space. We treat this as a narrow, generator-specific finding rather than a general claim that MEF-greedy selection preserves doctrinal alignment.
...
\textbf{No LLM policy evaluated.} This paper evaluates the random-sampling policies (\texttt{SelfPlayEngine}, \texttt{SampledBestResponsePolicy}) only. The repository already contains an LLM-backed policy (\texttt{LLMPolicy}, using a Claude API backend); the experiment script includes LLM evaluation infrastructure, but all 90 LLM calls returned connection errors in the offline evaluation environment, so the LLM column was omitted from reported results rather than substituted with a random fallback that would not represent LLM behavior. Evaluating \texttt{LLMPolicy} under the same protocol -- comparing its candidate diversity, doctrinal alignment, and wargame outcomes against the sampling baseline -- is the most important missing comparison and a clear next step.
...
Using only existing \texttt{coageneration} library code, we ran a 90-scenario self-play experiment comparing single-sample and sampled best-response COA generation, and found consistent evidence across four metrics (GBC, Nash gap, doctrinal alignment, and wargame win rate) that sampled best-response produces a measurably stronger and, in this generator, not-less-doctrinally-aligned opponent. We also found and fixed a methodological gap in which the benchmark's scenario-framing parameter had no effect on generated content, replacing a silent no-op with a small, honestly-described, currently-deterministic effect. The most important next step is evaluating the repository's \texttt{LLMPolicy} under the same protocol to compare LLM-generated COAs against the sampling baselines. We view this paper as a working harness and initial data point for that larger study, not as a finished evaluation of COA generation quality.
