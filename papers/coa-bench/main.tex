...
The experiment script writes three output files to \texttt{results/coa-bench/main/}: \texttt{rows.csv} (one row per scenario with all per-scenario metrics), \texttt{summary.csv} (the bootstrap-CI summary reported in Table~\ref{tab:main} and Sections~4.3--4.5), and \texttt{details.json} (run configuration including the model identifier and whether LLM policy was active). The random stream is keyed by base seed; all reported numbers are reproducible bit-for-bit given the same library version.
