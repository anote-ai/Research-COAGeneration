...
% Draft manuscript targeting DAI 2026. Replace rights, DOI, ISBN, conference
...
\begin{abstract}
Military course-of-action (COA) generation is a recurring planning task in which a force must propose, compare, and select among candidate plans under an adversarial response. We present COA-Bench, a small, fully offline, reproducible framework for studying COA generation policies under self-play. Following the terminology of the Transformational Model for Decision Advantage (TM-DA)~\cite{tmda2025bootcamp} -- an operational battle-management framework in which \emph{Match Effectors} (MEF) pairs required effects with candidate assets and \emph{Generate BattleCOA} (GBC) assembles those matches into a coherent, evaluable plan -- we adopt MEF and GBC as the names for COA-Bench's two core scoring mechanisms, while keeping their existing, unmodified mathematical definitions: a per-COA Mission Effectiveness Function score combining effectiveness, cost, and risk, and a pairwise Game-Based Comparison score measuring one force's advantage over another. This is a naming alignment, not a re-derivation: the scoring formulas are reused without modification from the \texttt{coageneration} library, and this paper reports no new algorithm for either quantity. The framework represents a COA as a typed, chainable sequence of actions with an associated MEF score, evaluates pairs of opposing COAs with a GBC score and Nash-gap distance, and scores doctrinal coherence with a heuristic FM 3-0-inspired rubric. We compare three RED response policies across 90 synthetic scenarios spanning three operational templates (urban stability, maritime interdiction, multi-domain combat): single random response, sampled best-of-8, and an LLM-backed policy using Claude (\path{claude-haiku-4-5-20251001}). Sampled best-response produces measurably stronger RED COAs by MEF score, lowering BLUE's wargame win rate from 0.900 to 0.811. The LLM policy produces the highest doctrinal alignment of any RED policy (0.804, approaching BLUE's 0.847) but the lowest MEF effectiveness -- BLUE wins \emph{more} against LLM RED (0.911) than against random single-sample RED (0.900). This decoupling -- high rubric score, low strategic effectiveness -- reveals that the FM 3-0 doctrinal rubric and the MEF score measure partially distinct properties, which is itself a benchmark diagnostic. We further show that the benchmark's scenario-framing parameter (blue / neutral / adversary), originally stored as scenario metadata only, previously had no effect on generated COA content; we trace this to specific generator and rubric interactions, wire framing into the generated objective text and action mix, and report a measured framing-sensitivity effect, the construction of which we describe explicitly because it is currently deterministic rather than sampled from a richer model. All metrics, scenarios, and policies used here are reused without modification from the existing \texttt{coageneration} library; this paper's only new artifacts are the experiment script, the framing wiring, the TM-DA terminology alignment, and this writeup. Given the moderate scenario count (N=90) and an unvalidated heuristic doctrinal rubric, we present these as a methods contribution and a set of preliminary, falsifiable findings rather than evidence about real-world COA quality.
\end{abstract}
...
This paper does not introduce a new benchmark from scratch, and it does not introduce a new scoring algorithm. The \texttt{coageneration} library already implements the typed COA representation, a self-play engine, a sampled best-response policy, bootstrap confidence intervals, a multi-COA comparison utility, an FM 3-0-inspired doctrinal rubric, and a stylized Lanchester-attrition wargame check, all built around two scalar quantities that this paper -- following an operational battle-management vocabulary -- names MEF and GBC. Our contribution in this paper is threefold: (1) we align COA-Bench's terminology with the Match Effectors / Generate BattleCOA vocabulary of the Transformational Model for Decision Advantage (TM-DA)~\cite{tmda2025bootcamp}, an internal battle-management briefing that independently uses the acronyms MEF and GBC for a related but distinct pair of DecisionFunctions, and document exactly where the analogy holds and where it does not (Section~\ref{sec:tmda}); (2) we exercise the existing \texttt{coageneration} machinery as a single coherent experiment rather than as isolated unit tests; and (3) we report the resulting numbers honestly, including a methodological gap we found and fixed along the way.
...
\textbf{Battle-management decision functions (TM-DA).} The Transformational Model for Decision Advantage (TM-DA) decomposes battle management into precisely defined \emph{DecisionFunctions}~\cite{tmda2025bootcamp}. Two of these are named Match Effectors (MEF) and Generate BattleCOA (GBC): MEF pairs a needed \emph{BattleEffect} with candidate \emph{Effectors}, producing ranked \texttt{EffectEffectorMatches}; GBC then weaves those matches, together with their resourcing, timing, and mutual-exclusion constraints, into a hypergraph of \texttt{BattleEvents} representing one or more physically possible, tactically valid \texttt{BattleCOAPaths}. This is a richer, graph-structured process than COA-Bench's scalar MEF score and pairwise GBC comparison, and COA-Bench does not implement TM-DA's hypergraph representation, its DecisionFunction sequencing, or its worldline-completeness criteria. We adopt the acronyms because the underlying intuition is analogous at a coarse grain -- MEF as a measure of how well a plan's constituent choices are matched to the effect they are meant to produce, GBC as the process/score by which a candidate BattleCOA is generated and judged relative to an opponent -- but we treat this as a terminology alignment for readability, not a claim of architectural equivalence. Section~\ref{sec:tmda} states this correspondence and its limits explicitly.

\textbf{LLM agent and planning evaluation.} ReAct interleaves reasoning and acting in language-model agents~\cite{yao2023react}, and AgentBench evaluates LLM agents end to end across interactive environments rather than on isolated completions~\cite{liu2024agentbench}. COA-Bench's typed action/chain representation, including conditional branches and tool calls, is designed so that the same evaluation pipeline can score both a programmatic sampling policy and an LLM-backed policy; this paper evaluates all three, comparing a random sampler, a best-of-$N$ sampler, and \texttt{LLMPolicy} (Claude) under the same protocol.
...
A COA is a typed object: an ordered or chained set of \texttt{Action}s (each with a category, target, priority, and optional tool call), an optional conditional branch, an objective string, a force assignment (BLUE or RED), a domain tag, and a scalar MEF score in $[-1, 1]$ combining effectiveness, cost, and risk. Conditional branches and tool calls let a COA represent doctrinally meaningful structures -- e.g., a reconnaissance action gating a strike-or-withdraw branch -- rather than a flat action list.

\subsection{MEF and GBC: Definitions and TM-DA Alignment}
\label{sec:tmda}

COA-Bench computes MEF as a weighted, clamped combination of three per-COA components:
\begin{equation}
\mathrm{MEF} = \mathrm{clamp}_{[-1,1]}\big(w_e \cdot \mathrm{effectiveness} - w_c \cdot \mathrm{cost} - w_r \cdot \mathrm{risk}\big),
\end{equation}
with default weights $w_e=0.5$, $w_c=0.3$, $w_r=0.2$. This is the same \texttt{compute\_mef\_score} formula that has been in the \texttt{coageneration} library since its first implementation; nothing about the arithmetic changes in this paper. Two opposing COAs are compared with the GBC score,
\begin{equation}
\mathrm{GBC} = \frac{\mathrm{MEF}_{\mathrm{blue}} - \mathrm{MEF}_{\mathrm{red}} + 2}{4} \in [0,1],
\end{equation}
likewise unchanged from \texttt{gbc\_score}.

Under TM-DA, MEF (\emph{Match Effectors}) and GBC (\emph{Generate BattleCOA}) name two sequential DecisionFunctions: MEF searches for and ranks candidate assets against a needed effect, and GBC assembles the resulting matches -- together with their resourcing, timing, and physical-possibility constraints -- into one or more complete \texttt{BattleCOAPath}s, which are themselves compared and refined~\cite{tmda2025bootcamp}. COA-Bench's MEF score is a coarse, single-scalar analogue of that first step: it measures how well a COA's chosen actions (its \emph{effector} choices) trade off effectiveness against cost and risk, without representing the underlying EffectEffectorMatch search or ranking process explicitly. COA-Bench's GBC score is a coarse, single-scalar analogue of the second step's comparison function: it measures one force's MEF advantage over an opponent's, without representing TM-DA's hypergraph assembly, worldline completeness, or mutual-exclusion adjudication. We use the TM-DA names because they describe the same two conceptual moments in COA generation -- matching an action's contribution to an effect, and generating/comparing complete plans built from those contributions -- but we do not claim COA-Bench implements TM-DA's DecisionFunction semantics, and readers should not infer that COA-Bench's MEF or GBC scores are compatible with, or produce the same outputs as, a TM-DA-conformant implementation. Extending COA-Bench toward the richer hypergraph representation TM-DA describes is future work, not something this paper attempts.
...
\begin{figure*}[t]
  ...
  \Description{Three bar charts side by side. Left: GBC score (blue advantage), where sampled best-of-8 has the lowest value and LLM has the highest. Center: Nash gap, where LLM has the smallest gap and sampled best-of-8 the largest. Right: RED doctrinal alignment, where LLM is substantially higher than both sampling policies.}
  \label{fig:policy}
\end{figure*}
...
The Nash gap for LLM RED (.150) is the smallest of the three -- meaning the LLM's payoff is closest to a balanced exchange against BLUE's fixed COA. This is not because the LLM found a strong equilibrium response, but because its lower MEF pulls the payoff split toward the center. We treat this pattern -- high rubric score, low MEF, small Nash gap -- as a benchmark diagnostic: it reveals that the doctrinal rubric and MEF are measuring different things, and that an LLM-backed policy can score well on one without improving on the other. It is also a reminder of the TM-DA framing above: an EffectEffectorMatch that reads as doctrinally coherent is not the same as one that has been ranked highly against the effect it is meant to produce, and COA-Bench's coarse MEF score, like TM-DA's MEF DecisionFunction, is meant to measure the latter.

\subsection{Multi-COA Comparison}

Applying \texttt{compare\_coas} to the 8 sampled RED candidates per scenario gives a mean candidate diversity (mean pairwise Jaccard distance of action types) of .732 [.720, .745], a mean MEF spread of .254 [.245, .264], and a mean of 2.15 [1.98, 2.33]\footnote{Exact bootstrap bounds: [1.978, 2.333]; rounded to two decimal places in text.} Pareto-optimal candidates out of 8 on the (MEF, doctrinal alignment) frontier. These numbers indicate that the 8 sampled candidates are not redundant: roughly a quarter of them are simultaneously non-dominated on effectiveness and doctrinal alignment, which is the situation \texttt{compare\_coas} is designed to surface for a human planner choosing among several proposed COAs rather than a single auto-selected one -- the same role TM-DA assigns to GBC's presentation of multiple \texttt{BattleCOAPath}s for a battle manager to consider, though \texttt{compare\_coas} ranks a flat candidate list rather than a path hypergraph. Figure~\ref{fig:pareto} shows the (MEF, doctrinal alignment) scatter for the first 10 scenarios, illustrating the spread within each candidate set and the lack of a single dominant corner.
...
\begin{figure}[t]
  ...
  \Description{Scatter plot of MEF score on the x-axis versus doctrinal alignment on the y-axis for 80 candidate COAs across 10 scenarios. Points are spread across the space with no single dominant corner, indicating trade-offs between effectiveness and doctrinal coherence.}
  \label{fig:pareto}
\end{figure}
...
\textbf{MEF/GBC terminology is aligned, not architecturally equivalent.} Section~\ref{sec:tmda} states this explicitly, but it bears repeating here: COA-Bench's MEF and GBC are scalar scores computed by unmodified, pre-existing formulas in the \texttt{coageneration} library. TM-DA's MEF and GBC are structured DecisionFunctions operating over an EventSpace hypergraph. Any reader familiar with TM-DA should treat this paper's use of the acronyms as a deliberate, documented terminology choice for a much simpler artifact, not as an implementation or validation of TM-DA.
...
\textbf{Heuristic, unvalidated rubric.} \texttt{fm30\_rubric\_scores} is a keyword- and category-based feature extractor inspired by FM 3-0 planning criteria, not a substitute for scoring by doctrine experts. Several of the saturation effects documented above (e.g., a single fixed action unconditionally satisfying the risk-mitigation term) show that the rubric can be insensitive to genuinely important plan content. Any claim about ``doctrinal alignment'' in this paper should be read as ``score under this heuristic rubric,'' not as a validated doctrinal judgment.

\textbf{Framing wiring is a proof of concept, not a validated manipulation.} The framing fix described above demonstrates that framing \emph{can} be made to matter, using the smallest change that produces a measurable, non-zero, honestly-reported effect. It is not a claim that real scenario framing in military planning has this specific structure or magnitude of effect on plan quality. A follow-up should replace the fixed one-action injection with a richer, sampled, scenario-template-aware framing model.

\textbf{Single LLM and single prompt evaluated.} The LLM comparison uses one model (\path{claude-haiku-4-5-20251001}) and one fixed system prompt. Different models, prompt phrasings, or chain-of-thought scaffolding would likely produce different doctrinal alignment and MEF profiles. The finding that LLM RED scores high on the rubric but low on MEF is specific to this model--prompt combination and this simulator's MEF function; it should not be generalized to claim that LLMs cannot find effective COAs under different evaluation setups. Comparing multiple LLMs and prompts, and aligning the MEF function with LLM-legible reward signals, is important future work.

\textbf{Simulated, not operational.} All outcomes come from the offline \texttt{coageneration} simulator. No live planner, real intelligence data, or operational system was used or affected. This benchmark performs no real-world action and contains no personal, classified, or proprietary data; all scenarios are synthetic.

\textbf{Broader impact and ethics.} COA-Bench operates in a domain -- military operations planning -- where automated systems carry heightened accountability requirements. The scenarios are fictional constructs that do not describe any real operation, location, or force. COA-Bench is an evaluation harness, not a planning assistant: it is not connected to any command system and is not intended to support real operational decision-making. Methods for automated adversarial reasoning can, in principle, be adapted for misuse; the action model and scenario space are deliberately kept at an abstracted, synthetic level that encodes no operational specifics or intelligence about real targets. The TM-DA document cited for terminology is itself an unclassified, distribution-A training briefing describing a conceptual model, not an operational system or targeting data; we cite it only for its DecisionFunction naming and definitions. Finally, this paper explicitly documents the measurement gap we found (framing had no effect on generated content) and the limits of the TM-DA terminology analogy rather than omitting either -- an especially important norm in military AI research, where overstating evaluation quality can have real consequences if methods are prematurely deployed.

\section{Conclusion}

Using the \texttt{coageneration} library, we ran a 90-scenario self-play experiment comparing three RED policies: single random sampling, best-of-8 sampling, and an LLM policy (Claude Haiku), and we aligned the benchmark's existing MEF and GBC scores with the Match Effectors / Generate BattleCOA terminology of TM-DA while leaving their formulas untouched. Sampled best-response is the strongest RED policy by MEF and wargame win rate; LLM RED produces the highest doctrinal alignment (.804) but lower strategic effectiveness than even the single random baseline, revealing a decoupling between the FM 3-0 rubric and MEF that is itself informative about what each metric captures. We also found and fixed a methodological gap in which the scenario-framing parameter had no effect on generated COA content. We view this as a working harness for COA policy evaluation -- a reproducible starting point for the next studies: richer scenario corpora, multiple LLM models and prompts, MEF functions legible to LLM-style optimization, and, longer term, a closer architectural alignment with TM-DA's hypergraph representation of BattleCOAs.
...
Claude (Anthropic) materially assisted with software implementation, experiment scripting, and manuscript drafting, including diagnosing the scenario-framing no-op described in Section~4.7 and drafting the TM-DA terminology-alignment language in Section~\ref{sec:tmda}. The authors are responsible for reviewing the code, verifying the generated results against the released artifacts, checking all citations -- including confirming attribution for the TM-DA source before submission -- and approving the final claims and text.
...
The experiment script writes three output files to \path{results/coa-bench/main/}: \texttt{rows.csv} (one row per scenario with all per-scenario metrics), \texttt{summary.csv} (the bootstrap-CI summary reported in Table~\ref{tab:main} and Sections~4.4--4.7), and \texttt{details.json} (run configuration including the model identifier and whether LLM policy was active). The random stream is keyed by base seed; all reported numbers are reproducible bit-for-bit given the same library version.

To activate the LLM RED policy, set the \texttt{ANTHROPIC\_API\_KEY} environment variable before running the experiment:
...
Without the key the LLM columns are filled with \texttt{NaN} and only the two sampling-policy columns are reported. The 30 scenarios without an API key and the 90 scenarios reported in the paper use the same random seeds, so partial runs are still comparable.

To verify that a completed run matches every number cited in this paper within numerical tolerance, run:

\begin{verbatim}
python experiments/check_coa_bench_results.py
\end{verbatim}

The script reads \texttt{summary.csv}, checks all 13 metrics against their reported mean and 95\% CI bounds (tolerance $\pm 0.002$), and prints \textsc{pass}/\textsc{fail} for each. A clean run prints \texttt{All 13 checks passed}.
