...
The implementation, experiment script, and raw results are available in this repository at \texttt{experiments/coa/run\_benchmark.py} and \texttt{results/coa/coa-bench/main/}.
...
The BattleCOA Boot Camp material describes two relevant DecisionFunctions: asset-effect matching (COA matching), which searches for BattleAssets that can achieve a requested BattleEffect and returns ranked EffectEffectorMatches, and course-of-action generation (COA generation), which uses those matches as seeds for constructing BattleCOA hypergraphs. Our current implementation is a simplified research harness and does not implement either DecisionFunction directly. Earlier drafts used COA matching and COA generation as names for internal scalar metrics; this draft avoids that collision and calls them the synthetic COA quality score and BLUE-vs-RED advantage score.
...
The experiment script writes \texttt{results/coa/coa-bench/main/rows.csv} (one row per scenario), \texttt{summary.csv} (the bootstrap-CI summary reported in Table~\ref{tab:main} and Section~4.3--4.4), and \texttt{details.json} (run configuration). The random stream is keyed by base seed; all reported numbers are reproducible bit-for-bit given the same library version.
