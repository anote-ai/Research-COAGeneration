...
\begin{abstract}
Course-of-action (COA) generation requires producing structured plans whose quality depends on adversarial response, force availability, and qualitative planning constraints. We introduce COA-Bench, an offline benchmark for comparing COA generation policies through self-play. Following BattleCOA terminology, we reserve COA matching for asset-effect matching and COA generation for course-of-action generation; the benchmark does not implement those DecisionFunctions directly. Instead, it represents COAs as typed action chains, assigns a synthetic COA quality score, compares opposing COAs with a BLUE-vs-RED advantage score and Nash-gap distance, and evaluates doctrinal coherence with a heuristic FM 3-0-inspired rubric. In 150 synthetic scenarios spanning five operational templates, a sampled best-response policy that draws eight RED candidates reduces BLUE advantage from .518 to .486 and BLUE wargame win rate from .900 to .820 relative to a single-sample response. An offline LLM-policy baseline using the same JSON contract reaches .496 BLUE advantage and .700 BLUE win rate, and a two-stage multi-agent council obtains .509 BLUE advantage and .813 BLUE win rate. A three-rater rubric-validation study over 600 rating records yields .978 inter-rater agreement. The contribution is an inspectable evaluation harness and preliminary benchmark result, not an operational planner or evidence about real-world military decision quality.
\end{abstract}
...
\begin{itemize}
    ...
    \item empirical evidence over 150 scenarios that sampled adversarial response, an offline LLM-policy responder, and multi-agent council generation change win-rate and plan-quality conclusions, plus a rubric-validation check and a benchmark-development finding showing that scenario framing was a silent no-op until wired into generated content.
\end{itemize}
...
The benchmark includes deliberately simple baselines. The single-sample RED policy estimates how a COA looks under shallow opposition. The quality-greedy best-of-$k$ policy measures sensitivity to adversarial search budget. The doctrine-aware policy controls for the possibility that high synthetic quality is not the same as a coherent plan. The offline LLM-policy baseline uses the same JSON schema and parser as a live model-backed policy, but a deterministic local completion client replaces the external model for reproducibility. The council policy then changes the BLUE generator itself by adding proposal, critique, repair, and adjudication. These baselines are calibration points that make later live LLM-backed or learned policies comparable under the same trace schema.

\subsection{Technical Implementation}

The implementation separates scenario construction, policy generation, and evaluation into typed modules. Scenarios are Pydantic objects containing assets, force labels, terrain metadata, and seed COAs. Policies share a common interface, so the evaluator can swap a single-sample baseline, sampled best response, doctrine-aware selector, offline LLM-policy responder, or multi-agent council without changing the experiment driver. This modularity is important for benchmarking because a live LLM policy can replace the offline completion client while preserving identical scenarios, response budgets, metrics, and CSV schemas.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Metric & Single & Quality & Doctrine & LLM-pol. \\
\midrule
BLUE advantage & .518 & .486 & .489 & .496 \\
Nash gap & .198 & .260 & .255 & .241 \\
RED doctrine & .730 & .731 & .758 & 1.000 \\
...
Table~\ref{tab:main} shows that sampled best-response produces a stronger RED opponent. BLUE advantage falls from .518 to .486, and BLUE wargame win rate falls from .900 to .820. The doctrine-aware selector produces a similar adversarial effect (.489 advantage, .807 BLUE win rate) while exposing a different selection objective. The offline LLM-policy responder is not a live model result; it is a reproducible contract test for the LLM wrapper. It produces highly structured responses, which raises RED doctrine to 1.000 and lowers BLUE win rate to .700.

The candidate set also contains nontrivial variation. Across sampled RED candidates, mean action-type diversity is .681, mean quality-score spread is .255, and an average of 2.17 of 8 candidates are Pareto-optimal on the quality and doctrinal-alignment frontier. The doctrine-aware selector changes the chosen response in 54\% of scenarios, incurring .036 quality regret while gaining .099 doctrinal alignment. Table~\ref{tab:budget} shows monotonic adversarial pressure: BLUE advantage drops from .518 at $k=1$ to .503 at $k=2$, .492 at $k=4$, .486 at $k=8$, and .480 at $k=16$.
