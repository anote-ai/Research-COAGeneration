...
def main() -> None:
    ...
    result_dir = root / "results/aaai2027/executable"
    summaries = _rows(result_dir / "summary.csv")
    ablations = _rows(root / "results/aaai2027/ablations/summary.csv")
    ...
    challenge = _rows(root / "results/aaai2027/challenge/summary.csv")
    with (root / "results/aaai2027/challenge/comparisons.csv").open(
        newline="", encoding="utf-8"
    ) as handle:
        challenge_comparisons = {
            row["baseline"]: row for row in csv.DictReader(handle)
        }
    paper = (root / "papers/aaai2027/main.tex").read_text(encoding="utf-8")
